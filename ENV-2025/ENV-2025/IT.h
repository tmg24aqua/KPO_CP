...
#define TI_CHAR_DEFAULT	0x00			 // значение по умолчанию для char
...
#define RANDOM_PARAMS_CNT 1				 // кол-во параметров у функции random

namespace IT			// таблица идентификаторов
{
	enum IDDATATYPE { INT = 1, CHAR = 2, PROC = 3, UNDEF = 4 };	// типы данных: целочисленный, символьный, процедура, неопределенный
	enum IDTYPE { V = 1, F = 2, P = 3, L = 4, S = 5 };			// типы идентификаторов: переменная, функция, параметр, литерал, стандартная функция
	enum STDFNC { F_COMPARE, F_RANDOM, F_NOT_STD };				// стандартные функции: сравнение строк, произвольная

	// Параметры стандартных функций
	static const IDDATATYPE COMPARE_PARAMS[] = { IT::IDDATATYPE::CHAR, IT::IDDATATYPE::CHAR };
	...
	struct Entry	// строка таблицы идентификаторов
	{
		...
		union
		{
			...
			struct
			{
				...
				char str[STR_MAXSIZE - 1];	// символы строки
			} vstr;						// значение строкового литерала
			char vsymbol;				// значение символьного литерала
			struct
			{
		...
		Entry()							// конструктор без параметров
		{
			...
			this->value.vsymbol = TI_CHAR_DEFAULT;
		};
